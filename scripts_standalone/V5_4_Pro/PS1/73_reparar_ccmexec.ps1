...
$LogFile = Join-Path $LogDir ("v5_pro_73_reparar_ccmexec_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Reparar CCMExec"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Reparar CCMExec'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-Service CcmExec -ErrorAction SilentlyContinue | Format-List * | Out-File (Join-Path $OutputFolder '73_ccmexec_antes.txt') -Encoding UTF8
