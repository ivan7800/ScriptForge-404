<#
============================================================
ScriptForge 404 - V5 Pro - Forzar sincronizacion Intune
...
$LogFile = Join-Path $LogDir ("v5_pro_23_forzar_sync_intune_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Forzar sincronizacion Intune"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Forzar sincronizacion Intune'
    ...
    if ($false -and -not $DryRun) { Restart-Service IntuneManagementExtension -Force -ErrorAction SilentlyContinue; Start-Process 'ms-settings:workplace';  } else { Write-Log 'Modo diagnostico.' }
}
catch {
