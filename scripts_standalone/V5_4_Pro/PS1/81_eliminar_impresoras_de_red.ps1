...
$LogFile = Join-Path $LogDir ("v5_pro_81_eliminar_impresoras_red_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Eliminar impresoras de red"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Eliminar impresoras de red'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-Printer | Export-Csv (Join-Path $OutputFolder '81_printers_backup.csv') -NoTypeInformation -Encoding UTF8
