...
$LogFile = Join-Path $LogDir ("v5_pro_75_limpiar_cache_sccm_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Limpiar caché SCCM"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Limpiar caché SCCM'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-WmiObject -Namespace root\ccm\SoftMgmtAgent -Class CacheInfoEx -ErrorAction SilentlyContinue | Export-Csv (Join-Path $OutputFolder '75_sccm_cache.csv') -NoTypeInformation -Encoding UTF8
