<#
============================================================
ScriptForge 404 - V5.4 Pro - Reparar WebView2 Runtime
...
$LogFile = Join-Path $LogDir ("v5_pro_58_reparar_webview2_runtime_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Reparar WebView2 Runtime"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Reparar WebView2 Runtime'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-ChildItem 'HKLM:\SOFTWARE\Microsoft\EdgeUpdate\Clients' -ErrorAction SilentlyContinue | ForEach-Object { Get-ItemProperty $_.PSPath } | Where-Object { $_.name -match 'WebView' } | Export-Csv (Join-Path $OutputFolder '58_webview2.csv') -NoTypeInformation -Encoding UTF8
