...
$LogFile = Join-Path $LogDir ("v5_pro_76_exportar_logs_sccm_completo_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Exportar logs SCCM completo"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Exportar logs SCCM completo'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    $d=Join-Path $OutputFolder '76_sccm_logs'
    New-Item $d -ItemType Directory -Force | Out-Null
    Copy-Item 'C:\Windows\CCM\Logs\*.log' $d -ErrorAction SilentlyContinue
    if('true' -eq 'true'){ Compress-Archive $d (Join-Path $OutputFolder '76_sccm_logs.zip') -Force }
