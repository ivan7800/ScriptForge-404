@echo off
:: ============================================================
:: ScriptForge 404 - V5.4 Pro - Reiniciar Intune Management Extension
...
set "LOGFILE=%LOGDIR%\v5_pro_68_reiniciar_intune_management_extension_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Reiniciar Intune Management Extension >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Reiniciar Intune Management Extension
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Reiniciar Intune Management Extension
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
sc query IntuneManagementExtension > "%OUT%\68_ime_antes.txt" 2>&1
