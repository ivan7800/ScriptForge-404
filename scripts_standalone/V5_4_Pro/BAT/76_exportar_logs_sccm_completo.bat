@echo off
:: ============================================================
:: ScriptForge 404 - V5.4 Pro - Exportar logs SCCM completo
...
set "LOGFILE=%LOGDIR%\v5_pro_76_exportar_logs_sccm_completo_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Exportar logs SCCM completo >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Exportar logs SCCM completo
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Exportar logs SCCM completo
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
powershell -NoProfile -ExecutionPolicy Bypass -Command "$d='%OUT%\76_sccm_logs'; New-Item $d -ItemType Directory -Force | Out-Null; Copy-Item 'C:\Windows\CCM\Logs\*.log' $d -ErrorAction SilentlyContinue"
