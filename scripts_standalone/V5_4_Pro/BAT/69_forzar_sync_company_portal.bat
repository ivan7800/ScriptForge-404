...
set "LOGFILE=%LOGDIR%\v5_pro_69_forzar_sync_company_portal_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Forzar sync Company Portal >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Forzar sync Company Portal
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Forzar sync Company Portal
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
schtasks /query /fo LIST /v | findstr /i EnterpriseMgmt > "%OUT%\69_enterprisemgmt_tasks.txt"
