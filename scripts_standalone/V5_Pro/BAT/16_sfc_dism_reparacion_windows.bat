...
set "LOGFILE=%LOGDIR%\v5_pro_16_sfc_dism_reparacion_windows_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5 Pro - SFC DISM reparacion Windows >> "%LOGFILE%" 2>nul

echo Ejecutando: V5 Pro - SFC DISM reparacion Windows
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5 Pro - SFC DISM reparacion Windows
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
DISM /Online /Cleanup-Image /ScanHealth > "%OUT%\dism_scanhealth.log"
echo RestoreHealth omitido.
