<#
============================================================
ScriptForge 404 - V5 Pro - SFC DISM reparacion Windows
Categoria: Sistema | Nivel de riesgo: MEDIO
...
$LogFile = Join-Path $LogDir ("v5_pro_16_sfc_dism_reparacion_windows_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - SFC DISM reparacion Windows"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - SFC DISM reparacion Windows'
    ...
    if ($false -and -not $DryRun) { DISM /Online /Cleanup-Image /RestoreHealth | Out-File (Join-Path $OutputFolder 'dism_restorehealth.log') }
