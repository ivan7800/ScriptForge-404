...
$LogFile = Join-Path $LogDir ("v5_pro_28_diagnostico_defender_bitlocker_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Diagnostico Defender BitLocker"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Diagnostico Defender BitLocker'
