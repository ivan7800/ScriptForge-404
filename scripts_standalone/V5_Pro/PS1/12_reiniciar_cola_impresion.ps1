<#
============================================================
ScriptForge 404 - V5 Pro - Reiniciar cola de impresion
Categoria: Impresoras | Nivel de riesgo: MEDIO
...
$LogFile = Join-Path $LogDir ("v5_pro_12_reiniciar_cola_impresion_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Reiniciar cola de impresion"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Reiniciar cola de impresion'
    ...
    if ($false -and -not $DryRun) { Stop-Service Spooler -Force; Remove-Item "$env:windir\System32\spool\PRINTERS\*" -Force -ErrorAction SilentlyContinue; Start-Service Spooler } else { Write-Log 'Modo diagnostico.' }
}
catch {
