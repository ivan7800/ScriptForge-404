<#
============================================================
ScriptForge 404 - V5 Pro - Reparar cliente SCCM
...
$LogFile = Join-Path $LogDir ("v5_pro_24_reparar_cliente_sccm_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Reparar cliente SCCM"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Reparar cliente SCCM'
    ...
    $CcmSetup='\servidor\SCCMCLIENT\ccmsetup.exe'
    $Site='P01'
    if ($false -and -not $DryRun) { if(Test-Path $CcmSetup){  Start-Process $CcmSetup -ArgumentList "SMSSITECODE=$Site" -Wait } else { throw 'No existe ccmsetup.exe: ' + $CcmSetup } } else { Write-Log 'Modo diagnostico.' }
}
catch {
