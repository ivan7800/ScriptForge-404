@echo off
:: ============================================================
:: ScriptForge 404 - V5.4 Pro - Eliminar impresoras de red
...
set "LOGFILE=%LOGDIR%\v5_pro_81_eliminar_impresoras_red_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Eliminar impresoras de red >> "%LOGFILE%" 2>nul
...
echo Ejecutando: V5.4 Pro - Eliminar impresoras de red
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Eliminar impresoras de red
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
powershell -NoProfile -Command "Get-Printer | Export-Csv '%OUT%\81_printers_backup.csv' -NoTypeInformation"
