...
set "LOGFILE=%LOGDIR%\v5_pro_12_reiniciar_cola_impresion_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5 Pro - Reiniciar cola de impresion >> "%LOGFILE%" 2>nul

echo Ejecutando: V5 Pro - Reiniciar cola de impresion
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5 Pro - Reiniciar cola de impresion
