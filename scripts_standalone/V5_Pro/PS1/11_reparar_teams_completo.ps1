...
$LogFile = Join-Path $LogDir ("v5_pro_11_reparar_teams_completo_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Reparar Teams completo"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Reparar Teams completo'
    ...
    Get-ItemProperty HKLM:\Software\Microsoft\EdgeUpdate\Clients\* -ErrorAction SilentlyContinue | Where-Object {$_.name -like '*WebView*'} | Out-File (Join-Path $OutputFolder 'webview2.txt')
    Copy-Item "$env:APPDATA\Microsoft\Teams\logs.txt" $OutputFolder -ErrorAction SilentlyContinue
    if ($false -and -not $DryRun) { Get-Process ms-teams,msteams,Teams -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue; Remove-Item \"$env:LOCALAPPDATA\Packages\MSTeams_8wekyb3d8bbwe\LocalCache\Microsoft\MSTeams\" -Recurse -Force -ErrorAction SilentlyContinue; Start-Process 'ms-teams:' } else { Write-Log 'Modo diagnostico.' }
}
catch {
