@echo off
:: ============================================================
:: ScriptForge 404 - V5.4 Pro - Reparar LoadBehavior Teams Add-in
...
set "LOGFILE=%LOGDIR%\v5_pro_60_reparar_loadbehavior_teams_addin_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Reparar LoadBehavior Teams Add-in >> "%LOGFILE%" 2>nul
...
echo Ejecutando: V5.4 Pro - Reparar LoadBehavior Teams Add-in
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Reparar LoadBehavior Teams Add-in
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
reg query "HKCU\Software\Microsoft\Office\Outlook\Addins\TeamsAddin.FastConnect" > "%OUT%\60_teams_addin_antes.txt" 2>&1
