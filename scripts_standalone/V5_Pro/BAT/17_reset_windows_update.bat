@echo off
:: ============================================================
:: ScriptForge 404 - V5 Pro - Reset Windows Update
:: Categoria: Windows Update | Nivel de riesgo: ALTO
...
set "LOGFILE=%LOGDIR%\v5_pro_17_reset_windows_update_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5 Pro - Reset Windows Update >> "%LOGFILE%" 2>nul

echo Ejecutando: V5 Pro - Reset Windows Update
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5 Pro - Reset Windows Update
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
sc query wuauserv > "%OUT%\wu_services_antes.txt"
echo [MODO DIAGNOSTICO] Sin reset.
