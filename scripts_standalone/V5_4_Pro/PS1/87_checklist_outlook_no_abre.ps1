...
$LogFile = Join-Path $LogDir ("v5_pro_87_checklist_outlook_no_abre_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Checklist Outlook no abre"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Checklist Outlook no abre'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    $items = '1. Abrir en modo seguro;2. Revisar add-ins;3. Reset navpane;4. Revisar perfil;5. Revisar OST/PST;6. Escalar con eventos'.Split(';')
    $html = $items | ForEach-Object { '<li>' + $_ + '</li>' }
    ('<h1>TICKET-0000 - Outlook no abre</h1><ul>' + ($html -join '') + '</ul>') | Set-Content (Join-Path $OutputFolder '87_checklist-outlook-no-abre.html') -Encoding UTF8
