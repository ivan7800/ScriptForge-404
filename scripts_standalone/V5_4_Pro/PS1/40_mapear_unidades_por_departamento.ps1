...
$LogFile = Join-Path $LogDir ("v5_pro_40_mapear_unidades_por_departamento_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Mapear unidades por departamento"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Mapear unidades por departamento'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Write-Log 'Departamento solicitado: CAU'
    $Table = @('CAU|P|\servidor\cau
    CAU|S|\servidor\software
    RRHH|R|\servidor\rrhh'.Split([Environment]::NewLine))
