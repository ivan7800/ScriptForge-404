...
$LogFile = Join-Path $LogDir ("v5_pro_42_desconectar_todas_unidades_red_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Desconectar todas las unidades de red"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Desconectar todas las unidades de red'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    cmd /c 'net use' | Out-File (Join-Path $OutputFolder '42_net_use_backup.txt') -Encoding UTF8
