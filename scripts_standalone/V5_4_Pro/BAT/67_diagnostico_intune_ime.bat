@echo off
:: ============================================================
:: ScriptForge 404 - V5.4 Pro - Diagnóstico Intune IME
...
set "LOGFILE=%LOGDIR%\v5_pro_67_diagnostico_intune_ime_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Diagnóstico Intune IME >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Diagnóstico Intune IME
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Diagnóstico Intune IME
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
sc query IntuneManagementExtension > "%OUT%\67_ime_service.txt" 2>&1
