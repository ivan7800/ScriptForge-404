@echo off
:: ============================================================
:: ScriptForge 404 - V5.4 Pro - Exportar drivers impresora
...
set "LOGFILE=%LOGDIR%\v5_pro_83_exportar_drivers_impresora_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Exportar drivers impresora >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Exportar drivers impresora
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Exportar drivers impresora
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
pnputil /enum-drivers > "%OUT%\83_pnputil_drivers.txt"
