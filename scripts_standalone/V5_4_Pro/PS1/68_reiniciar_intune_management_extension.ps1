...
$LogFile = Join-Path $LogDir ("v5_pro_68_reiniciar_intune_management_extension_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Reiniciar Intune Management Extension"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Reiniciar Intune Management Extension'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-Service IntuneManagementExtension -ErrorAction SilentlyContinue | Format-List * | Out-File (Join-Path $OutputFolder '68_ime_antes.txt') -Encoding UTF8
