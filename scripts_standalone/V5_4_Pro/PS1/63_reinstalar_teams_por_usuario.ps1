...
$LogFile = Join-Path $LogDir ("v5_pro_63_reinstalar_teams_usuario_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Reinstalar Teams por usuario"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Reinstalar Teams por usuario'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    winget list Microsoft.Teams | Out-File (Join-Path $OutputFolder '63_winget_teams.txt') -Encoding UTF8
