...
$LogFile = Join-Path $LogDir ("v5_pro_48_reparar_outlook_no_abre_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Reparar Outlook no abre"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Reparar Outlook no abre'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-Process OUTLOOK -ErrorAction SilentlyContinue | Export-Csv (Join-Path $OutputFolder '48_outlook_process.csv') -NoTypeInformation -Encoding UTF8
