...
set "LOGFILE=%LOGDIR%\v5_pro_59_ver_rutas_teams_addin_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Ver rutas Teams Add-in >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Ver rutas Teams Add-in
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Ver rutas Teams Add-in
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
dir "%LOCALAPPDATA%\Microsoft\TeamsMeetingAdd-in" /s > "%OUT%\59_teams_addin_paths.txt" 2>&1
