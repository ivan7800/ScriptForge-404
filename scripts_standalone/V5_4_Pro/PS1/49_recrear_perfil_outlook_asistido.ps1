<#
============================================================
ScriptForge 404 - V5.4 Pro - Recrear perfil Outlook asistido
...
$LogFile = Join-Path $LogDir ("v5_pro_49_recrear_perfil_outlook_asistido_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Recrear perfil Outlook asistido"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Recrear perfil Outlook asistido'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-ChildItem 'HKCU:\Software\Microsoft\Office\16.0\Outlook\Profiles' -ErrorAction SilentlyContinue | Select-Object PSChildName | Export-Csv (Join-Path $OutputFolder '49_profiles.csv') -NoTypeInformation -Encoding UTF8
