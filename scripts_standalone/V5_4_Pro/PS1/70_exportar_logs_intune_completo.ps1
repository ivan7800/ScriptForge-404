...
$LogFile = Join-Path $LogDir ("v5_pro_70_exportar_logs_intune_completo_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Exportar logs Intune completo"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Exportar logs Intune completo'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    $d=Join-Path $OutputFolder '70_intune_logs'
    ...
    if('true' -eq 'true'){ Compress-Archive $d (Join-Path $OutputFolder '70_intune_logs.zip') -Force }
