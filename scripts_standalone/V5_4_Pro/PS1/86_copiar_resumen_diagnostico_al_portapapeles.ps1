...
$LogFile = Join-Path $LogDir ("v5_pro_86_copiar_resumen_diagnostico_portapapeles_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Copiar resumen diagnóstico al portapapeles"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Copiar resumen diagnóstico al portapapeles'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    $summary = "Equipo: $env:COMPUTERNAME`nUsuario: $env:USERNAME`nIncidencia: Pendiente de completar`nFecha: $(Get-Date)"
