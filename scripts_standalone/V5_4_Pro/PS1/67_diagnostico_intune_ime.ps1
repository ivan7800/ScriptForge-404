...
$LogFile = Join-Path $LogDir ("v5_pro_67_diagnostico_intune_ime_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Diagnóstico Intune IME"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Diagnóstico Intune IME'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-Service IntuneManagementExtension -ErrorAction SilentlyContinue | Format-List * | Out-File (Join-Path $OutputFolder '67_ime_service.txt') -Encoding UTF8
