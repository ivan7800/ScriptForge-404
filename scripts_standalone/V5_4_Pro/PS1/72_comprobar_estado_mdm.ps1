...
$LogFile = Join-Path $LogDir ("v5_pro_72_comprobar_estado_mdm_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Comprobar estado MDM"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Comprobar estado MDM'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    dsregcmd /status | Out-File (Join-Path $OutputFolder '72_dsregcmd.txt') -Encoding UTF8
