...
$LogFile = Join-Path $LogDir ("v5_pro_25_exportar_logs_sccm_intune_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Exportar logs SCCM Intune"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Exportar logs SCCM Intune'
    ...
    Compress-Archive -Path (Join-Path $OutputFolder '*') -DestinationPath (Join-Path $OutputFolder 'logs_sccm_intune.zip') -Force
}
catch {
