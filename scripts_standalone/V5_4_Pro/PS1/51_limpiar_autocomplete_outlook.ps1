...
$LogFile = Join-Path $LogDir ("v5_pro_51_limpiar_autocomplete_outlook_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Limpiar Autocomplete Outlook"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Limpiar Autocomplete Outlook'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    $files=Get-ChildItem "$env:LOCALAPPDATA\Microsoft\Outlook\RoamCache" -Filter 'Stream_Autocomplete*.dat' -ErrorAction SilentlyContinue
