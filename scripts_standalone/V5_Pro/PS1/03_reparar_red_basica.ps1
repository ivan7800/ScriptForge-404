...
$LogFile = Join-Path $LogDir ("v5_pro_03_reparar_red_basica_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Reparar red basica"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Reparar red basica'
    ...
    if ($false -and -not $DryRun) {
        ...
        Write-Log "Reinicio de adaptador no solicitado."
