...
set "LOGFILE=%LOGDIR%\v5_pro_77_diagnostico_cliente_sccm_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Diagnóstico cliente SCCM >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Diagnóstico cliente SCCM
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Diagnóstico cliente SCCM
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
sc query CcmExec > "%OUT%\77_ccmexec.txt" 2>&1
