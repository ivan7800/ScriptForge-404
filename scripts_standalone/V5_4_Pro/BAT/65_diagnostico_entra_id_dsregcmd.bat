...
set "LOGFILE=%LOGDIR%\v5_pro_65_diagnostico_entra_id_dsregcmd_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Diagnóstico Entra ID dsregcmd >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Diagnóstico Entra ID dsregcmd
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Diagnóstico Entra ID dsregcmd
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
dsregcmd /status > "%OUT%\65_dsregcmd_status.txt"
