...
$LogFile = Join-Path $LogDir ("v5_pro_43_backup_unidades_mapeadas_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Backup unidades mapeadas"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Backup unidades mapeadas'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    cmd /c 'net use' | Out-File (Join-Path $OutputFolder '43_net_use.txt') -Encoding UTF8
