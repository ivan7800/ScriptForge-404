...
set "LOGFILE=%LOGDIR%\v5_pro_58_reparar_webview2_runtime_%date:~-4%%date:~3,2%%date:~0,2%_%time:~0,2%%time:~3,2%.log"
set "LOGFILE=%LOGFILE: =0%"
echo [%date% %time%] Inicio de script: V5.4 Pro - Reparar WebView2 Runtime >> "%LOGFILE%" 2>nul

echo Ejecutando: V5.4 Pro - Reparar WebView2 Runtime
echo.
set "OUT=C:\CAU\ScriptForge404"
if not exist "%OUT%" mkdir "%OUT%" >nul 2>&1
echo V5.4 Pro - Reparar WebView2 Runtime
echo Equipo: %COMPUTERNAME% ^| Usuario: %USERNAME%
echo Salida: %OUT%
reg query "HKLM\SOFTWARE\Microsoft\EdgeUpdate\Clients" /s | findstr /i WebView > "%OUT%\58_webview2_registry.txt" 2>&1
