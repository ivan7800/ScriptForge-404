...
$LogFile = Join-Path $LogDir ("v5_pro_18_diagnostico_office_365_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5 Pro - Diagnostico Office 365"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5 Pro - Diagnostico Office 365'
    ...
    $ospp = Get-ChildItem "$env:ProgramFiles\Microsoft Office" -Filter ospp.vbs -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1; if ($ospp) { cscript.exe //nologo $ospp.FullName /dstatus | Out-File (Join-Path $OutputFolder 'office_license.txt') }
}
catch {
