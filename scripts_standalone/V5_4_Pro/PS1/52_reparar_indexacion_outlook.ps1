...
$LogFile = Join-Path $LogDir ("v5_pro_52_reparar_indexacion_outlook_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
Write-Log "Ejecutando: V5.4 Pro - Reparar indexación Outlook"

try {
    $OutputFolder = 'C:\CAU\ScriptForge404'
    if (-not (Test-Path $OutputFolder)) { New-Item -ItemType Directory -Path $OutputFolder -Force | Out-Null }
    Write-Log 'V5.4 Pro - Reparar indexación Outlook'
    Write-Log ('Equipo: ' + $env:COMPUTERNAME + ' | Usuario: ' + $env:USERNAME)
    Write-Log ('Salida: ' + $OutputFolder)
    Get-Service WSearch -ErrorAction SilentlyContinue | Format-List * | Out-File (Join-Path $OutputFolder '52_wsearch.txt') -Encoding UTF8
